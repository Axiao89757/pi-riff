...
test("active Agent timing uses yellow while completed turns stay purple", async () => {
	...
	assert.match(stripTerminalControls(activeMessage), /^\[1\] \d+(?:\.\d)?s \/ \d+(?:\.\d)?s$/);
...
test("agent timing entries show compact turn and cumulative duration", () => {
	...
	assert.equal(line, "[4] 12s / 1m 15s | 2026.7.27 17:11");
	assert.ok(rawLine.includes("\x1b[1;38;2;109;40;217m[4] 12s\x1b[0m"));
